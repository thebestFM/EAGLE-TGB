...
from utils.utils import compute_metrics, set_random_seed
...
class TPPRNodeFinder:
    ...
    def update_tppr(self, source: int, target: int):
        # Update TPPR for source after get edge
...
def main():
    ...
    tppr_finder = TPPRNodeFinder(num_nodes=num_nodes, k=args.topk, alpha=args.tppr_alpha, beta=args.tppr_beta)
    ...
    for split in ["train", "val", "test"]:
        ...
        result = run_epoch(loaders[split], forecaster, tppr_finder, args.topk, args.gamma, metrics, dataset, device)
